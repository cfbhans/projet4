...
<h1>Valider vos collaborateurs</h1>
    <h2>Liste des inscrits :</h2>
        ...
        foreach($users as $user) {
            if(!$user->isAdmin()){
            ?>
            <form class="input-group mb-3" method="post" action="<?= cf_link('users/' . $user->getId() . '/setAdmin'); ?>" >
                ...
                ?>
             <form class="input-group mb-3" method="post" action="<?= cf_link('users/' . $user->getId() . '/unsetAdmin'); ?>" >
